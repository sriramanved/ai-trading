...
class PrioritizedReplayBuffer(BaseBuffer):
    ...
    def add(self, obs: np.ndarray, next_obs: np.ndarray, action: np.ndarray, reward: np.ndarray, done: np.ndarray, infos: List[Dict[str, Any]]) -> None:
        ...
        self.priorities[self.pos - 1] = self.max_priority  # Assign max priority to new experience

    def sample(self, batch_size: int, beta: float = 0.4) -> ReplayBufferSamples:
        if self.full:
        ...
        return ReplayBufferSamples(*samples, weights=weights, indices=indices)
